...
class Guild_User(Base):
    ...
    id: Mapped[intid]
    disc_id: Mapped[int] = mapped_column(ForeignKey("users.c.disc_id"))
    guild_id: Mapped[int] = mapped_column(ForeignKey("guilds.c.guild_id"))
...
class Guilds(Base):
    ...
    count_of_members: Mapped[int] = mapped_column(nullable=True)
    guild_sets: Mapped[str] = mapped_column(default=lambda x: await JsonEncoder.code_to_json(GUILD_CONFIG))
    ...
    def __repr__(self) -> str:
        return f"id={self.id!r}, guild_id={self.guild_id!r}, guild_name={self.guild_name!r}, count_of_members={self.count_of_members!r}, guild_settings={self.guild_sets!r}"
