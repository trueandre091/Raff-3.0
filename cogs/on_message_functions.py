...
import counter_functions


async def reactions_thread_check(message, settings_dict) -> None:

    if message.channel.id in settings_dict["CHANNELS"]:
...
async def boosts_check(message, settings_dict) -> None:

    if message.type == disnake.MessageType.application_command and message.author.id in settings_dict["BOOST_BOTS"].values():
